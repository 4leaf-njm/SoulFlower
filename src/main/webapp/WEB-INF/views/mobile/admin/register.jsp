				...
				<div class="item_wrap scrollbar-outer">
					<div class="item" id="item1">
						...
						<div class="row">
							<div class="col-md">
								<label>리베이트</label>
								<span class="result01">0 원</span>
							</div>
							<div class="col-md">
								<label>수익</label>
								<span class="result02">0 원</span>
							</div>
						</div>
						<div class="row">
							<div class="col-lg">
...
<script>
	...
	function itemEvent() {
		...
		$('.btn_reg').click(function(event) {
			...
			var html = '';
			html += '<div class="item" data-no="' + count + '">';
			html += '	<div class="row">';
			...
			html += '			<label>가격</label>';
			html += '			<span>' + comma(itemPrice) + ' 원</span>';
			html += '		</div>';
			html += '		<div class="col-md">';
			html += '			<label>수량</label>';
			html += '			<input type="text" class="txt_updateAmount" value="' + amount + '" style="width: 55px; padding: 2px 3px; border: 1px solid #dedede; font-size: 13px; text-align: center;"/>';
			html += '     		<a href="#" class="btn_updateAmount"><i class="fas fa-check" style="padding: 0 0 0 10px; color: #ff5959; font-size: 15px;"></i></a></td>';
			html += '		</div>';
			html += '	</div>';
			html += '	<div class="row">';
			html += '		<div class="col-md">';
			html += '			<label>리베이트</label>';
			html += '			<span>' + comma(rebate) + ' 원</span>';
			html += '		</div>';
			html += '		<div class="col-md">';
			html += '			<label>수익</label>';
			html += '			<span>' + comma(profit) + ' 원</span>';
			html += '		</div>';
			html += '	</div>';
			html += '	<div class="row">';
			html += '		<div class="col-lg">';
			html += '			<button type="button" class="btn_remove">삭제</button>';
			html += '		</div>';
			html += '	</div>';
			html += '</div>';
			$('.item_final .items').append(html);
			removeEvent();
	...
	function updateAmountClick() {
		$('.btn_updateAmount').click(function(event) {
			...
			var itemPrice = parseInt($parent.find('.row').eq(1).find('.col-md').eq(0).find('span').text().replace(' 원', '').replace(/,/gi, ''));
			...
			$parent.find('.row').eq(2).find('.col-md').eq(0).find('span').text(comma(rebate) + ' 원');
			$parent.find('.row').eq(2).find('.col-md').eq(1).find('span').text(comma(profit) + ' 원');
	...
	$('.btn_addItem').click(function(event){
		event.preventDefault();
		var $item = $('.item_wrap .item');
		console.log($item.last());
		var lastItemNo = $item.last().attr("id").replace("item", "");
        var newitem = $item.eq(0).clone();
        newitem.find('input:radio[class=rd_rebate]').attr('name', 'rd_rebate' + (parseInt(lastItemNo)+1));
        newitem.append('<a href="#" class="btn_removeItem"><i class="fas fa-minus"></i></a>');
        var html = '<div class="item" id="item' + (parseInt(lastItemNo)+1) + '">' + newitem.html() + '</div>';
	...
	function removeEvent() {
		$('.item_final .btn_remove').click(function(event) {
			event.stopImmediatePropagation();
			event.preventDefault();
			var no = $(this).parents('.item').data('no');
